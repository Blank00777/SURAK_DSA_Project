...
def merge_sort_by_price(stocks: List[StockData]) -> List[StockData]:
    ...
    left_half = merge_sort_by_price(stocks[:mid])
    right_half = merge_sort_by_price(stocks[mid:])

    # Conquer: merge the sorted halves
    return _merge(left_half, right_half)


def _merge(left: List[StockData], right: List[StockData]) -> List[StockData]:
    """
    Helper function to merge two sorted lists into one sorted list.

    Args:
        left: Sorted list of StockData objects
        right: Sorted list of StockData objects
...
def calculate_prefix_sum_prices(stocks: List[StockData]) -> List[float]:
    ...
    # First element is just the first price
    prefix.append(stocks[0].price)

    # Calculate cumulative sums
    for i in range(1, len(stocks)):
        prefix.append(prefix[i-1] + stocks[i].price)
...
def calculate_moving_average(stocks: List[StockData], window_size: int) -> List[Tuple[str, float]]:
    ...
    """
    # Handle edge cases
    if not stocks or window_size < 1 or window_size > len(stocks):
        return []
    ...
    # Calculate moving averages starting from the first valid window
    for i in range(window_size - 1, len(stocks)):
        # Calculate average using prefix sums
        if i == window_size - 1:
            # First window: sum from start to i
            window_sum = prefix[i]
        else:
            # Subsequent windows: sum from (i - window_size + 1) to i
            window_sum = prefix[i] - prefix[i - window_size]

        # Calculate average and round to 2 decimal places
        avg = round(window_sum / window_size, 2)

        # Append (date, average) tuple
        result.append((stocks[i].date, avg))
...
def kadane_max_profit(stocks: List[StockData]) -> Tuple[int, int, float]:
    ...
    """
    # Handle edge case: need at least 2 prices to have a buy/sell
    if len(stocks) < 2:
        return (0, 0, 0.0)
    ...
    max_sum = changes[0]           # Global maximum sum
    current_sum = changes[0]       # Current subarray sum
    max_start = 0                  # Start index of max subarray (in changes array)
    max_end = 0                    # End index of max subarray (in changes array)
    current_start = 0              # Start index of current subarray
    ...
    # Step 3: Convert indices back to stock indices
    # Buy at the beginning of the max subarray (in stocks array)
    buy_index = max_start

    # Sell at the end of the max subarray + 1 (in stocks array)
    sell_index = max_end + 1

    # Calculate actual profit
    max_profit = stocks[sell_index].price - stocks[buy_index].price
